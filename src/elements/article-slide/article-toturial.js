...
import previewStyles from "./article-toturial.css?inline";
import { litStaticStyles } from "../utils.js";

export class ArticleToturial extends LitElement {
  static properties = {
  ...
  handleToturialSlotChange(event) {
    this.toturialStepTargets = event.target
      .assignedElements({ flatten: true })
      .flatMap((element) => [element, ...element.querySelectorAll("*")])
      .filter((element) => element.hasAttribute("step") || "step" in element || element.localName.includes("-"));
    this.updateToturialStep();
  }
  ...
  updateToturialStep() {
    const step = this.activeIndex;
    this.toturialStepTargets.forEach((element) => {
      element.step = step;
  ...
  render() {
    const activeStep = String(this.activeIndex + 1).padStart(2, "0");
    ...
    return html`
      ...
      <div class="slider">
        ...
        <section class="toturial-view">
          <div class="step">${activeStep}</div>
          <slot name="toturial" @slotchange=${(event) => this.handleToturialSlotChange(event)}></slot>
        </section>
      </div>
      <ul class="bullets">
        ${Array.from(
...
if (!customElements.get("article-toturial")) {
  customElements.define("article-toturial", ArticleToturial);
}
